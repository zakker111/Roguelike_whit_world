...
export function createEquipmentOfSlot(slot, tier, rng) {
  const r = getRng(rng);
...
export function typesBySlot(slot) {
  return Object.values(TYPES).filter(t => t.slot === slot);
}

export function pickType(slot, tier, rng) {
...
export function createEquipment(tier, rng) {
  const r = getRng(rng);
...
export function addType(slot, def) {
...
export function createByKey(key, tier, rng, overrides) {
...
export function createNamed(config, rng) {
