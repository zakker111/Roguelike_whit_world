...
function startGmMarkerEncounter(ctx, encounterId) {
  if (!ctx) return false;
  ...
  // IMPORTANT (Phase 1 fix): marker actions must be ctx-first for mode transitions.
  try {
    if (ctx.Modes && typeof ctx.Modes.enterEncounter === "function") {
      ok = !!ctx.Modes.enterEncounter(ctx, tmpl, biome, difficulty);
    }
  } catch (_) {}
  ...
  if (!ok) {
    try { if (ctx && typeof ctx.log === "function") ctx.log("[GM] Failed to start marker encounter.", "warn"); } catch (_) {}
    return false;
  }
