/**
 * GMRuntime simulation core (Node + browser compatible).
 *
 * This file contains the deterministic assertion suite that used to live in
 * `/<repo>/gm_sim.js` (which also had DOM rendering).
 *
 * - `runGmSim()` returns a JSON report and does not touch the DOM.
 * - Browser pages can import this and render however they like.
 * - Node scripts / CI can import this and fail the process if `report.ok === false`.
 */

import * as GMRuntime from "../runtime.js";
import { ensureScheduler } from "../runtime/state_ensure.js";
...
export function runGmSim() {
  ...
  checks.push(
    check("8) scheduler queue dedupe preserves insertion order", () => {
      const ctx = createCtx();
      const gm = resetAndPrime(ctx, { turn: 0, mode: "world" });

      const sched = ensureScheduler(gm);

      // Dedupe should keep the first occurrence and discard later duplicates.
      sched.actions = {
        a: { id: "a", createdTurn: 1, status: "scheduled" },
        b: { id: "b", createdTurn: 2, status: "scheduled" },
      };
      sched.queue = ["a", "b", "a", "b", "a"];

      ensureScheduler(gm);
      assertEq(gm.scheduler.queue.join(","), "a,b", "queue should keep first occurrence order when deduping");

      // If an action exists but is missing from the queue (legacy/corrupt state), it should
      // be appended deterministically.
      gm.scheduler.actions = {
        b: { id: "b", createdTurn: 5, status: "scheduled" },
        a: { id: "a", createdTurn: 1, status: "scheduled" },
      };
      gm.scheduler.queue = ["b"];

      ensureScheduler(gm);
      assertEq(gm.scheduler.queue.join(","), "b,a", "missing action should append after existing queue in deterministic order");

      return { queue1: ["a", "b"], queue2: ["b", "a"] };
    })
  );
