/**
 * Region tile cache helpers and tilesRef().
 */
...
export function fallbackFillRegion(WT, id) {
  try {
...
export function fillRegionFor(WT, id) {
  cacheResetIfNeeded();
  const k = id | 0;
  let v = TILE_CACHE.fill[k];
  if (v) return v;
  const td = getTileDef("region", id);
  v = (td && td.colors && td.colors.fill) ? td.colors.fill : fallbackFillRegion(WT, id);
  TILE_CACHE.fill[k] = v;
  return v;
}

export function glyphRegionFor(id) {
  cacheResetIfNeeded();
  const k = id | 0;
