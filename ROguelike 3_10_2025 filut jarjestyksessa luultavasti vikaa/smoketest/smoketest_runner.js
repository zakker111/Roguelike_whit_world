    ...
    try {
      ...
      try {
        if (window.GameAPI && typeof window.GameAPI.getMode === "function" && window.GameAPI.getMode() === "dungeon") {
          ...
          if (!handledDP && false) {
            recordSkip("Dungeon persistence scenario module not available; skipping inline fallback");
          }
          ...
          // 9b: equip best items from inventory (if any) and test manual equip/unequip
          {
            ...
            if (!handledInv && false) {
              recordSkip("Inventory scenario module not available; skipping inline fallback");
            }
          }
          ...
          // 9c: force enemy spawn in dungeon and verify presence/types
          {
            ...
            if (!handledCombat && false) {
              recordSkip("Combat scenario module not available; skipping inline fallback");
            }
          }
