...
export function buildBaseTown(ctx) {
  ...
  // Delegate to TownState.deriveTownBiomeFromWorld so all callers share the same heuristic.
  (function deriveTownBiome() {
    try {
      ...
      const wx = (ctx.worldReturnPos && typeof ctx.worldReturnPos.x === "number")
        ...
        : ((world.originX | 0) + (ctx.player ? (ctx.player.x | 0) : 0));
      const wy = (ctx.worldReturnPos && typeof ctx.worldReturnPos.y === "number")
        ? (ctx.worldReturnPos.y | 0)
        : ((world.originY | 0) + (ctx.player ? (ctx.player.y | 0) : 0));

      const townsArr = (ctx.world && Array.isArray(ctx.world.towns)) ? ctx.world.towns : [];
      const rec = townsArr.find(t => t && (t.x | 0) === wx && (t.y | 0) === wy) || info || null;

      // If biome already pinned on the town record, just mirror into ctx.
      if (rec && rec.biome) {
