...
- POIs (towns/dungeons/castles) are placed sparsely and deterministically; density slightly increased (~1–2%).
- Castle settlements:
  - Extremely rare overworld POIs (CASTLE tiles) that behave like large towns but are flagged kind: "castle".
  - Prefer coasts/rivers when generated by the infinite world; dungeon/ruins balance is unchanged.
  - Rendered as 'C' glyphs in a distinct castle color on both the main map and minimap (separate from town colors); inside, town generation reserves a central keep and adds a few extra guards.
- Travelling caravans:
...
- Default encounter rate is 50% (tunable via the GOD panel “Encounter rate” slider; persists), tuned to produce roughly one overworld encounter every 20–35 movement tiles.
...
- Multi-faction encounters:
  - Some templates pit factions against each other (e.g., Night Raid: goblins vs bandits, Guards vs Bandits: guards vs bandits on an open field).
  - In Guards vs Bandits, the battle takes place on a wide, obstacle-free battlefield: guards and bandits spawn in large lines near opposite edges, with a clear gap so you can watch both sides begin their charge.
  - You spawn off to the side; guards begin neutral and will not attack you unless provoked. Bumping a neutral guard shows a centered confirm dialog ("Do you want to attack the guard? This will make all guards hostile to you.") before you can strike.
  - If you let the fight play out and neutral guards win (all bandits dead, guards still neutral), they celebrate with a one-time "For the kingdom!" log before the usual "Area clear" message.
- Caravan ambush:
  - Bumping into a caravan "c" on the overworld asks if you want to encounter it; confirming starts a caravan_ambush road encounter featuring a Caravan master merchant and guard-only enemies.
  - Guards start neutral and only become hostile if you attack them; many caravan guards are tougher than normal and may carry healing potions they drink at low HP.
  - A special caravan chest on the road holds better-than-normal loot and a substantial gold reward. After resolving the ambush, you can accept an escort job from the Caravan master to travel with the caravan to its destination for gold.
...
- Town chests and lockpicking:
  ...
  - Successful attempts open the chest, grant gold and occasional equipment, and increment your lockpicking skill; failures still advance skill slightly and leave the chest in place.
  - Basic lockpicks come from starting gear, loot, and shops; fine lockpicks are rarer, decay more slowly, and can also appear in encounter chests.
